...
import { getAllClients, getClient, getClientTasks } from "@/lib/firestore";
import type { ClientProfile, Task } from "@/lib/types";
import AdminNav from "@/components/AdminNav";
import ClientInfoTab from "@/components/ClientInfoTab";
import TaskCalendarTab from "@/components/TaskCalendarTab";
...
export default function ClientViewPage() {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([getClient(clientId), getClientTasks(clientId)]).then(([cp, t]) => {
      setClientProfile(cp);
      setTasks(t);
      setLoading(false);
    });
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <div className="bg-white border-b border-gray-100 px-6 py-2 flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm text-gray-400">
          ...
          <span>/</span>
          <span className="text-gray-500">Portal View</span>
        </div>
              ...
              <div className="absolute right-0 top-full mt-1 w-52 bg-white rounded-xl border border-gray-100 shadow-lg z-20 overflow-hidden">
                {allClients.map(c => (
                  <button key={c.clickupFolderId}
                    onClick={() => { router.push(`/clients/${c.clickupFolderId}/view`); setSwitcherOpen(false); }}
