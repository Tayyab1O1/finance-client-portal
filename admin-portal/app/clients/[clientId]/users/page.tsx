...
import { getClient, getUsersForClient, createUserRecord } from "@/lib/firestore";
...
export default function ClientUsersPage() {
  ...
  async function loadData() {
    const [c, u] = await Promise.all([getClient(clientId), getUsersForClient(clientId)]);
    setClient(c);
    setUsers(u);
    setLoading(false);
  }
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    try {
      ...
      await createUserRecord(cred.user.uid, email, clientId);
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 px-8 py-8 max-w-2xl">
        <div className="flex items-center gap-2 text-sm text-gray-400 mb-6">
          ...
          <span>/</span>
          <Link href={`/clients/${clientId}/edit`} className="hover:text-[#1a1a2e] transition">{displayName}</Link>
          <span>/</span>
